...
for (let pic of picArray) {
  ...
  headEle.innerText = pic.title;

  // <img src="medium_image_from_picArray" alt="title_from_picArray"/>
  let imgEle = document.createElement('img');
  imgEle.src = pic.image.medium;
  imgEle.alt = pic.title;

  // <figcaption>caption_from_picarray</figcaption>
  let figCapEle = document.createElement('figcaption');
  figCapEle.innerText = pic.caption;
  ...
  pEle.innerText = pic.description;

  let articleEle = document.createElement('article');
  articleEle.className = 'card';
  articleEle.appendChild(headEle);
  articleEle.appendChild(figureEle);
  articleEle.appendChild(pEle);

  let dialogEle = document.querySelector('dialog');
  articleEle.addEventListener('click', function(evt){
    const imgDialog = dialogEle.querySelector('img');
    imgDialog.src = pic.image.large;
    imgDialog.alt = pic.title;

    dialogEle.showModal();
  });

  let spanDialog = dialogEle.querySelector('span');
  spanDialog.addEventListener('click', function(evt){
    dialogEle.close();
  });

  pictures.appendChild(articleEle);
}